...
可killable的阻塞地带超时地等待完成量。条件满足，返回剩余时间；超时，返回0；被中断，返回-ERESTARTSYS。
*/
...
static int __init wait_for_completion_killable_init(void){
	...
	printk("the value of done of the comple is: %d\n",comple.done);
	//-------------------	
	result = wait_for_completion_killable(&comple);
	printk("wait_for_completion(&comple) run !!!          -1 <----");
	printk("the value of done of the comple is: %d\n",comple.done);
	printk("the result of the wait_for_completion_killable is: %d\n",result);
