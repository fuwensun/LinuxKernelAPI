/********************************************************************************
* File Name:	round_jiffies_relative.c
* Description:	第7章实例训练          
...
{
	printk("<0>into round_jiffies_relative");
	unsigned long j=jiffies;//获取当前节拍数 	
	unsigned long __result1=__round_jiffies_relative(0,0); //第一个参数0代表相对节拍数，相对于当前的节拍，第二个参数0代表CPU编号
	unsigned long __result2=__round_jiffies_relative(0,1); //第一个参数0代表相对节拍数，相对于当前的节拍，第二个参数1代表CPU编号
	unsigned long result1=round_jiffies_relative(0);    //参数0代表相对节拍数,相对于当前的节拍
	unsigned long result2=round_jiffies_relative(0);//参数0代表相对节拍数,相对于当前的节拍
	...
	printk("<0>out round_jiffies_relative");
	return 0; 
} 

void __exit round_jiffies_relative_exit(void) 
{        
	printk("<0>Goodbye round_jiffies_relative\n"); 
} 
