...
static int __init enable_irq_wake_init(void) 
{
	...
	printk("<0>into enable_irq_wake_init\n");
	result=request_irq(irq,irq_handler,IRQF_DISABLED,"A_New_Device",NULL);//申请中断
	result1=enable_irq_wake(irq);  //调用enable_irq_wake()函数,可替换为set_irq_wake(irq,1);		
	printk("<0>the request_irq result is: %d\n",result);	//显示函数调用结果
	printk("<0>the enable_irq_wake result is :%d\n",result1);
	printk("<0>out enable_irq_wake_init\n");
	return 0;  
}
...
{ 
	...
	printk("<0>Goodbye enable_irq_wake\n");
 	return;
}  
