...
static int  __init  cdev_add_del_init(void);   //Ä£¿éŒÓÔØº¯ÊýÉùÃ÷
static void  __exit  cdev_add_del_exit(void);   //Ä£¿éÐ¶ÔØº¯ÊýÉùÃ÷
module_init(cdev_add_del_init);                 //Ä£¿éŒÓÔØ
module_exit(cdev_add_del_exit);                 //Ä£¿éÐ¶ÔØ
...
int __init cdev_add_del_init (void)
{
	...
	if (res)
	{
		cdev_del(mem_cdev);  //ÌíŒÓÊ§°Ü£¬ÉŸ³ýÉè±žÇý¶¯
		mem_cdev = NULL;     //ÌíŒÓÊ§°Ü£¬ÊÍ·Å»º³åÇø
		printk("<0>cdev add Error\n");
	}
	else
	{
...
{
	printk("<0>into cdev_add_del_exit\n");
	if (mem_cdev != NULL)
