/********************************************************************************
* File Name:	device_initialize_add_del.c
...
static int  __init  device_initialize_add_del_init(void);   //Ä£¿éŒÓÔØº¯ÊýÉùÃ÷
static void  __exit device_initialize_add_del_exit(void);   //Ä£¿éÐ¶ÔØº¯ÊýÉùÃ÷
module_init(device_initialize_add_del_init);                   //Ä£¿éŒÓÔØ
module_exit(device_initialize_add_del_exit);                 //Ä£¿éÐ¶ÔØ
...
int __init device_initialize_add_del_init(void)
{
	...
	device_initialize(dev);                //³õÊŒ»¯ÐÂŽŽœšµÄÂßŒ­Éè±ž
	printk("<0>device initialize success\n");
	...
	res=device_add(dev);   //œ«ÐÂŽŽœšµÄÂßŒ­Éè±žŒÓÈëLinuxÄÚºËÏµÍ³
	if(res)
	{
		printk("<0>device add failed\n");
		device_del(dev);  //Ê§°Ü£¬ÔòÉŸ³ýÂßŒ­Éè±ž
		kfree(dev);	   //Ê§°Ü£¬ÊÍ·ÅÂßŒ­Éè±žµÄÄÚŽæ¿ÕŒä
		return -1;
        }
	printk("<0>add device success\n");
	printk("<0>out device_initialize_add_del_init\n");
	return 0;
}
...
{
	printk("<0>into device_initialize_add_del_exit\n");	
	unregister_chrdev(MEM_MAJOR,"my_char_dev");   //×¢Ïú×Ö·ûÉè±ž
	if(dev)
	{
		put_device(dev);	 //ÂßŒ­Éè±žÒýÓÃŒÆÊýŒõ1
		device_del(dev);     //ÉŸ³ýÂßŒ­Éè±ž
	}
	printk("<0>device del success\n");	
	if(mem_class)
	...
	printk("<0>out device_initialize_add_del_exit\n");
} 
