...
static int  __init  get_put_device_init(void);   //Ä£¿éŒÓÔØº¯ÊýÉùÃ÷
static void  __exit get_put_device_exit(void);   //Ä£¿éÐ¶ÔØº¯ÊýÉùÃ÷
module_init(get_put_device_init);                 //Ä£¿éŒÓÔØ
module_exit(get_put_device_exit);                  //Ä£¿éÐ¶ÔØ
...
{
	int res;
	printk("<0>into get_put_device_init\n");
	...
	printk("<0>the reference count of the device is:%d\n",dev->kobj.kref.refcount);   //ÏÔÊŸµ±Ç°Éè±žµÄÒýÓÃŒÆÊý
	get_device(dev);   //ÔöŒÓÉè±žµÄÒýÓÃŒÆÊý
	printk("<0>after the get_device the reference count of the device is:%d\n",dev->kobj.kref.refcount); //ÏÔÊŸº¯Êýµ÷ÓÃÖ®ºóÉè±žµÄÒýÓÃŒÆÊý
	put_device(dev);   //ŒõÉÙÉè±žµÄÒýÓÃŒÆÊý
	printk("<0>after the put_device the reference count of the device is:%d\n",dev->kobj.kref.refcount);  //ÏÔÊŸº¯Êýµ÷ÓÃÖ®ºóÉè±žµÄÒýÓÃŒÆÊý
	printk("<0>out get_put_device_init\n");
	return 0;
}
...
{
	printk("<0>into get_put_device_exit\n");	
	...
	printk("<0>out get_put_device_exit\n");
}
