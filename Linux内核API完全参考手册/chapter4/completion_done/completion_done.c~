...
int my_function(void * argc)
{
	...
	result=completion_done(&comple); //²âÊÔcompletionÊÇ·ñÓÐµÈŽýÕß
	printk("<0>the return result of the completion_done is:%d\n",result); //ÏÔÊŸº¯Êýµ÷ÓÃœá¹û
...
{	
	int result;
	wait_queue_t data;
	printk("<0>into completion_done_init.\n");		
	...
	printk("<0>the current pid is:%d\n",current->pid); //ÏÔÊŸµ±Ç°œø³ÌµÄPIDÖµ
	printk("<0>out completion_done_init.\n");
	return 0;
}
...
{	
        printk("<0>Goodbye completion_done\n");	
}
