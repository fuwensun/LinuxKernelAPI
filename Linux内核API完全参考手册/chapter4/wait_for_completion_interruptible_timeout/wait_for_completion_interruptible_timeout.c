/********************************************************************************
* File Name:	wait_for_completion_interruptible_timeout.c
* Description:	第4章实例训练          
...
int my_function(void * argc)
{
	...
	printk("<0>the state of the real parent is :%ld\n",current->real_parent->state);   //ÏÔÊŸžžœø³ÌµÄ×ŽÌ¬
...
{	
	...
	long leavetime;
	wait_queue_t data;
	printk("<0>into wait_for_completion_interruptible_timeout_init.\n");				
	...
	leavetime=wait_for_completion_interruptible_timeout(&comple,1000);  //×èÈûœø³Ì£¬µÈŽýÐÂœø³ÌµÄœáÊø

	/*ÏÔÊŸº¯Êýwait_for_completion_interruptible_timeout( )µÄ·µ»Øœá¹û*/
	printk("<0>the result of the wait_for_completion_interruptible_timeout is:%ld\n",leavetime);	

	/*ÏÔÊŸº¯Êýkernel_thread( )µÄ·µ»Øœá¹û*/	
	printk("<0>the result of the kernel_thread is :%d\n",result);
	printk("<0>the current pid is:%d\n",current->pid);    //ÏÔÊŸµ±Ç°œø³ÌµÄPIDÖµ
	printk("<0>out wait_for_completion_interruptible_timeout_init.\n");
	return 0;
}
...
{	
        printk("<0>Goodbye wait_for_completion_interruptible_timeout\n");	
}
