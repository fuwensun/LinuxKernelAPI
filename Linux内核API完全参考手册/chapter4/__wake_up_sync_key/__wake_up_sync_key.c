/********************************************************************************
* File Name:	__wake_up_sync_key.c
* Description:	第4章实例训练          
...
int my_function(void * argc)
{
	...
	__wake_up_sync_key(&head,TASK_ALL,1,NULL); //µ÷ÓÃº¯Êý»œÐÑµÈŽý¶ÓÁÐÖÐµÄœø³Ì
	printk("<0>the state of the real_parent after __wake_up_sync_key is :%ld\n",current->real_parent->state); //ÏÔÊŸº¯Êýµ÷ÓÃÖ®ºóžžœø³ÌµÄ×ŽÌ¬
...
{			
	...
	long left_time=0;
	wait_queue_t data;
	printk("<0>into __wake_up_sync_key_init.\n");
	...
	printk("<0>the current pid is:%d\n",current->pid);   //ÏÔÊŸµ±Ç°œø³ÌµÄPIDÖµ
	printk("<0>out __wake_up_sync_key_init.\n");
	return 0;
}
...
{	
        printk("<0>Goodbye __wake_up_sync_key\n");	
}
